...
-- Set up LLDB as the debugger
dap.adapters.lldb = {
    type = "executable",
    command = "/usr/bin/lldb-vscode-11", -- Modify the path if necessary
    name = "lldb",
}

dap.virtual_text = true

-- Configure dap-ui and icons
...
-- Set up key mappings for dap and dap-ui

-- Set up the debug configuration for C with LLDB
dap.configurations.c = {
    {
        ...
        cwd = "${workspaceFolder}",
        stopOnEntry = true,
        args = {}, -- Modify this if you need to pass arguments to the program
        runInTerminal = false,
        env = {
...
-- Map a key to start a debug session for the current C file
vim.api.nvim_set_keymap("n", "<leader>dc", '<cmd>lua require("dap").run_last()<CR>', { noremap = true, silent = true })
